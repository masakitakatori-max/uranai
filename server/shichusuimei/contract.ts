...
import type { BaziRelation } from '../../src/lib/shichusuimeiTypes';
...
export function validateInterpretation(value: unknown, factIds: string[], sourceIds: string[], partner: boolean, luck: boolean, relations: BaziRelation[] = []) {
  ...
  const check = (entry: { evidenceIds: string[]; sourceIds: string[] }, person?: string, paired = false) => {
    ...
    const referenced = new Set(entry.evidenceIds);
    for (const relation of relations) if (referenced.has(relation.id)) {
      for (const id of [relation.fromId, relation.toId, ...(relation.memberIds || [])]) if (facts.has(id)) referenced.add(id);
    }
    if (person && ![...referenced].some(id => id.startsWith(person + '-'))) throw new Error('対象者の命式を根拠に含めていません');
    if (paired && !people.every(p => [...referenced].some(id => id.startsWith(p + '-')))) throw new Error('相性には二人の命式の根拠が必要です');
  };
